...
pub fn compare_records(
    ...
    let results = compare_record_distances(
        source,
        target,
        source_record,
        target_record,
...
impl CompareCommand {
    pub async fn execute(
        ...
        let target_compare_graph =
            CompareGraph::new(&target_graph, target_vecs_map);
        ...
        let (source_vectors, source_compare_graph) =
            if let Some(source_graph_dir) = self.source_graph_dir.as_ref() {
                ...
                let source_graph_path =
                    source_graph_dir_path.join("aggregated.graph");
        ...
        let candidates_for_compare = if self.source_graph_dir.is_some() {
            ...
            let field_graph =
                target_compare_graph.graph.get(&self.filter_field).unwrap();
            Either::Right(
                hnsw.knn(K_NEIGHBORS)
                    ...
                    .map(|(source_value_id, target_value_ids)| {
                        let all_target_record_ids: Vec<u32> = target_value_ids
                            .iter()
                            .flat_map(|id| {
                                field_graph.value_id_to_record_ids(*id)
                            })
                            .copied()
                            .collect();
        ...
        candidates_for_compare.try_for_each(
            |(sources, targets)| -> Result<(), anyhow::Error> {
                for source in sources {
                    for target in targets.iter() {
                        ...
                        if probability > self.match_threshold {
                            let source_id = source_compare_graph
                                .record_id_field_value(source);
                            let target_id = target_compare_graph
                                .record_id_field_value(*target);
