...
use crate::{
    graph::FullGraph, line_index::lookup_record, model::EmbedderMetadata,
...
impl Decider {
    async fn decide(&self, e1: &str, e2: &str) -> Result<bool, anyhow::Error> {
        ...
        match self {
            Decider::Interactive => {
                read_y_n().context("could not read y or n from stdin")
            }
            Decider::OpenAI(d) => {
...
impl GenerateMatchesCommand {
    pub async fn execute(
        ...
        let decider = if let Some(entity_description) =
            self.entity_description.as_ref()
        {
        ...
        let target_record_index_file_path =
            Path::new(&self.target_record_file_index);
        let target_record_index_file =
            File::open(target_record_index_file_path)
        ...
        let source_record_index_file_path =
            Path::new(&self.source_record_file_index);
        let source_record_index_file =
            File::open(source_record_index_file_path)
                .context("Could not open the source index file for records")?;
        ...
        let mut non_matches_csv_wtr =
            Writer::from_path(&self.non_matches_file)?;
        non_matches_csv_wtr.write_record(["source_id", "target_id"])?;

        let mut non_match_count = 0;
        let mut match_count = 0;

        let neighbor_results = source_vectors.iter().enumerate().map(
            |(source_vector_id, query_vec)| {
                (
        ...
        for (source_vector_id, neighbors) in neighbor_results {
            ...
            if i != 0 && i != neighbors.len() {
                let (target_vector_id, _distance) =
                ...
                let target_record_ids =
                    target_field_graph.value_id_to_record_ids(target_vector_id);
                let source_record_ids =
                    source_field_graph.value_id_to_record_ids(source_vector_id);
                for target_record_id in target_record_ids {
                    for source_record_id in source_record_ids {
                        ...
                        let source_id = source_graph
                            .record_id_field_value(*source_record_id);
                        let target_id = target_graph
                            .record_id_field_value(*target_record_id);
                        if is_match {
                            answers_csv_wtr
                                .write_record(&[source_id, target_id])?;
