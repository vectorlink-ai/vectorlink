...
fn get_record_position(record_number: usize, index_file: &mut File) -> (u64, u64) {
    ...
    index_file.seek(SeekFrom::Start(record_offset));
...
fn get_record(
    ...
    record_file.seek(SeekFrom::Start(start));
...
fn main() -> io::Result<()> {
    ...
    let search_file = File::open(args.search_file).expect("Can't open search file");
    ...
    for i in 0..num_records {
        ...
        let root_file_id = central_record.get("FILE_ID").unwrap().as_str().unwrap();
        let root_record_id = central_record.get("RECORD_ID").unwrap().as_str().unwrap();
        for _j in 0..args.match_length {
            let neighbor_id = index_file.read_u32::<NativeEndian>().unwrap();
            let neighbor_distance = index_file.read_f32::<NativeEndian>().unwrap();
            let neighbor_record =
                get_record(neighbor_id as usize, &mut record_file, &mut index_file);
            let file_id = neighbor_record.get("FILE_ID").unwrap().as_str().unwrap();
            let record_id = neighbor_record.get("RECORD_ID").unwrap().as_str().unwrap();
