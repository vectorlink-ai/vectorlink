...
fn get_record_position(
    ...
    let record_offset =
        record_number as u64 * std::mem::size_of::<u64>() as u64;
...
fn get_record(
    ...
    let json_string =
        std::str::from_utf8(&buf).expect("Not a valid json string");
...
fn main() -> io::Result<()> {
    ...
    let mut record_file =
        File::open(args.record_file).expect("Can't open ecord file");
    let mut index_file =
        File::open(args.record_index_file).expect("Can't open index file");
    let mut search_file =
        File::open(args.search_file).expect("Can't open search file");
    ...
    let mut output_table = Writer::from_path(args.output_table)
        .expect("could not open output table");
    output_table
        .write_record(&[
            "ROOT_DATAFILE_ID",
            "ROOT_ROW_ID",
    ...
    for i in 0..num_records {
        ...
        let root_file_id =
            central_record.get("DATAFILE_ID").unwrap().as_str().unwrap();
        let root_record_id =
            central_record.get("ROW_ID").unwrap().as_str().unwrap();
        for _j in 0..args.match_length {
            ...
            let neighbor_distance =
                search_file.read_f32::<NativeEndian>().unwrap();
            if neighbor_distance < args.threshold {
                let neighbor_record = get_record(
                ...
                let file_id = neighbor_record
                    ...
                    .unwrap();
                let record_id =
                    neighbor_record.get("ROW_ID").unwrap().as_str().unwrap();
