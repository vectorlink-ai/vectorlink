...
use crate::{
    comparator::{
        QuantizedVectorComparatorConstructor, VectorComparatorConstructor,
    },
    hnsw::Hnsw,
    layer::VectorComparator,
    memoize::{
        CentroidDistanceCalculatorConstructor, MemoizedCentroidDistances,
    },
...
impl<'a> VectorRangeIndexable for VectorRangeIndexableForVectors<'a> {
    fn get_ranges(&self, byte_ranges: &[Range<usize>]) -> Vectors {
        ...
        let mut data = SimdAlignedAllocation::alloc_zeroed(
            vector_count * vector_byte_size,
        );
        byte_ranges
            .into_par_iter()
...
pub fn centroid_finder<V: VectorRangeIndexable>(
    ...
    let range_count =
        vecs.num_vecs() * (vecs.vector_byte_size() / centroid_byte_size);
...
impl Quantizer {
    pub fn quantize<C: VectorComparator>(
        ...
        {
            let out_cast: &mut u16 =
                unsafe { &mut *(out.as_mut_ptr() as *mut u16) };
            *out_cast = self
                .hnsw
    ...
    pub fn reconstruct<C: VectorComparator>(
        &self,
        quantized: &[u8],
        vectors: &Vectors,
    ) -> Vec<u8> {
        let cast = unsafe {
            std::slice::from_raw_parts(
                quantized.as_ptr() as *const u16,
                quantized.len() * 2,
            )
        };
        let reconstructed_byte_size =
            quantized.len() * vectors.vector_byte_size();
...
>(
    ...
    let centroids =
        centroid_finder(vectors, centroid_count, centroid_byte_size, seed);
    eprintln!("found centroids");
    let centroid_comparator =
        CentroidComparatorConstructor::new_from_vecs(&centroids);
    let centroid_distance_calculator = CDC::new(&centroids);

    let mut centroid_hnsw =
    ...
    let quantized_vectors = quantizer.quantize_all(
        vectors.num_vecs(),
        vector_stream,
        &centroid_comparator,
    );
    ...
    let memoized_distances =
        MemoizedCentroidDistances::new(&centroid_distance_calculator);
    let quantized_comparator = QuantizedComparatorConstructor::new(
        &quantized_vectors,
        &memoized_distances,
    );
    let mut quantized_hnsw =
        Hnsw::generate(quantized_build_params, &quantized_comparator);
